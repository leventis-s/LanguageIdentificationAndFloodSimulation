...
/* RisingTides takes in a topography, a water height, and the sources
 * of the water, and returns information as to which places are
 * flooded.
 */



/* Creates a set of all neighboring squares of a given flooded square
 */
Set<GridLocation> getNeighbors(const GridLocation& place) {
    GridLocation south = {place.row + 1, place.col};
    GridLocation north = {place.row - 1, place.col};
    GridLocation east = {place.row, place.col + 1};
    GridLocation west = {place.row, place.col - 1};
    Set<GridLocation> neighborPlaces = {north, south, east, west};
    return neighborPlaces;
}

/* Create a grid with bool values in each location.
 * true corresponds to the location being flooded,
 * false means it is not flooded
 */
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            const Vector<GridLocation>& sources,
                            double height) {
    Grid<bool> floodedRegionsIn(terrain.numRows(), terrain.numCols());
    Queue<GridLocation> floodedLocations;
    Set<GridLocation> visitedLocations; // list will help ensure to not check the same spots twice
    for (GridLocation location: sources) {
        if (terrain.get(location) <= height) {
            floodedRegionsIn.set(location, true);  // flood location
            floodedLocations.enqueue(location); // add it to the queue to check its neighbors
            visitedLocations += location;
        }
    }
    while (!floodedLocations.isEmpty()) {
        GridLocation place = floodedLocations.dequeue();
        Set<GridLocation> neighborPlaces = getNeighbors(place);
        for (GridLocation neighbor: neighborPlaces) {
            if (!visitedLocations.contains(neighbor) && terrain.inBounds(neighbor)
               && terrain.get(neighbor) <= height) {
                    floodedRegionsIn.set(neighbor, true);
                    floodedLocations.enqueue(neighbor);
                    visitedLocations += neighbor;
            }
        }
    }
