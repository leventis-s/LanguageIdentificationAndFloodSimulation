...
using namespace std;
/* RosettaStone takes in a text and returns an educated guess
 * as to what language the text is in
 */


/* Counts the number of kGrams of n length and returns a map of
 * each kGram and its frequency
 */
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength < 1) {
        error("kGram Length should be a positive integer");
    }
    Map<string, double> kGramsIn;
    for (int i = 0; i + kGramLength < str.size() + 1; i++) { // loops until not enough chars for full kGram
...
/* Returns a new map identifcal to kGramsIn except values are normalized
 */
Map<string, double> normalize(const Map<string, double>& input) {
    Map<string, double> normalize;
    double squareSum = 0;
    for (string kGram: input) {
        squareSum += pow(input[kGram], 2);
    }
    if (squareSum == 0) {
        error("All kGrams have frequency of 0.");
    }
    for (string kGram: input) {
        normalize[kGram] = input[kGram]/sqrt(squareSum);
    }
    return normalize;
}

/* Returns a map with only the n number of most frequent kGrams
 */
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        error("Cannot keep a negative amount of kGrams.");
    }
    PriorityQueue<string> frequentkGrams;
    for (string kGram: source) {
        frequentkGrams.enqueue(kGram, source[kGram]);
    }
    for (int i = 0; i + numToKeep < source.size(); i++) {
        frequentkGrams.dequeue();   // take out n number of lowest weight kGrams
    }
    Map<string, double> topKGramsIn;
    while (!frequentkGrams.isEmpty()) {  // add the remaining kGrams to topKGram map
        string topKGram = frequentkGrams.dequeue();
        topKGramsIn[topKGram] = source[topKGram];
    }
    return  topKGramsIn;
}

/* Calculates the cosine similarity of the text with a certain
 * reference language document
 */
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    double sumFrequency = 0;
    for (string kGram: lhs) {
         if (rhs.containsKey(kGram)) {  // ensures kGram is in both maps
            sumFrequency += lhs[kGram] * rhs[kGram];
        }
    }
    return sumFrequency;
}

/* Returns a guess of what the language is
 */
string guessLanguageOf(const Map<string, double>& textProfile,
                       const Set<Corpus>& corpora) {
    if (corpora.isEmpty()) {
        error("No reference documents to identify language");
    }
    ...
    for (Corpus language: corpora) {
        ...
        if (similarity > bestSimilarity) {
            bestSimilarity = similarity;  // update best similarity value
            identifiedLanguage = language.name;  // update new language guess
        }
    }
